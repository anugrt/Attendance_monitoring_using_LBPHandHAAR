...
face_detector = cv2.CascadeClassifier('Multiple-Face-Recognition-master (1)\\haarcascade_frontalface_default.xml')
...
assure_path_exists("training_data/person2/")

# Looping starts here
while(True):

    # Capturing each video frame from the webcam
    t, image_frame = vid_cam.read()
    ...
    for (x,y,w,h) in faces:
        ...
        cv2.imwrite("training_data/person2/Person." + str(face_id) + '.' + str(count) + ".jpg", gray[y:y+h,x:x+w])
